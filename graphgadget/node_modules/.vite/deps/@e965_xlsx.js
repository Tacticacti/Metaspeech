...
function cc2str(arr, debomit) {
  ...
  var o = [], i = 0;
  try {
    for (i = 0; i < arr.length - 65536; i += 65536)
      o.push(String.fromCharCode.apply(0, arr.slice(i, i + 65536)));
    o.push(String.fromCharCode.apply(0, arr.slice(i)));
  } catch (e) {
    try {
      for (; i < arr.length - 16384; i += 16384)
        o.push(String.fromCharCode.apply(0, arr.slice(i, i + 16384)));
      o.push(String.fromCharCode.apply(0, arr.slice(i)));
    } catch (e2) {
      for (; i != arr.length; ++i)
...
function str_remove_ng(str, s, e) {
  ...
  while (si > -1) {
    ...
    if (ei == -1)
      break;
    if ((si = str.indexOf(s, last = ei + e.length)) == -1)
...
function str_match_xml(str, tag) {
  ...
  while (si >= 0 && si <= L - w && !xml_boundary[str.charAt(si + w)])
  ...
  if (f === "dbcs") {
    ...
    if (typeof $cptable !== "undefined" && current_ansi == 874) {
      ...
      if (cpp.length == val.length) {
        for (i = 0; i < val.length; ++i)
          if (cpp[i] == 0 && val.charCodeAt(i) != 0)
            cpp[i] = 95;
      }
      if (cpp.length == 2 * val.length) {
        for (i = 0; i < val.length; ++i)
          if (cpp[2 * i] == 0 && cpp[2 * i + 1] == 0 && val.charCodeAt(i) != 0)
            cpp[2 * i] = 95;
      }
    ...
    function finish_cell() {
      ...
      if (s.length === 0)
        ...
        if (_re && s.match(_re)) {
          ...
          if (o && o.UTC === false)
            v = utc_to_local(v);
        } else if (o && o.UTC === false)
          v = utc_to_local(v);
        else if (o.cellText !== false && o.dateNF)
          cell.w = SSF_format(cell.z, v);
        if (o.cellDates) {
          cell.t = "d";
          cell.v = v;
        } else {
...
function read_wb_ID(d, opts) {
  ...
  try {
    ...
    o.WTF = OLD_WTF;
    if (e.message.indexOf("SYLK bad record ID") == -1 && OLD_WTF)
      throw e;
        ...
        switch (cell.t) {
          case "n":
            if ((cell.v | 0) == cell.v && cell.v >= -32768 && cell.v <= 32767)
              write_biff_rec(ba, 13, write_INTEGER(R, C, cell));
            else
              write_biff_rec(ba, 14, write_NUMBER(R, C, cell));
            break;
          case "d":
            var dc = datenum(cell.v);
            if ((dc | 0) == dc && dc >= -32768 && dc <= 32767)
              write_biff_rec(ba, 13, write_INTEGER(R, C, { t: "n", v: dc, z: cell.z || table_fmt[14] }));
            else
              write_biff_rec(ba, 14, write_NUMBER(R, C, { t: "n", v: dc, z: cell.z || table_fmt[14] }));
            break;
          default:
          ...
          case 3074:
            {
              ...
              if (s["!data"] != null) {
                ...
                s[encode_col(C) + encode_row(R)] = { t: "s", v: str };
            }
            break;
          default:
            break;
...
var parse_sty_xml = /* @__PURE__ */ function make_pstyx() {
  return function parse_sty_xml2(data, themes, opts) {
    ...
    var t;
    if (t = str_match_xml_ns(data, "numFmts"))
      parse_numFmts(t[0], styles, opts);
    if (t = str_match_xml_ns(data, "fonts"))
      parse_fonts(t[0], styles, themes, opts);
    if (t = str_match_xml_ns(data, "fills"))
      parse_fills(t[0], styles, themes, opts);
    if (t = str_match_xml_ns(data, "borders"))
      parse_borders(t[0], styles, themes, opts);
    if (t = str_match_xml_ns(data, "cellXfs"))
      parse_cellXfs(t[0], styles, opts);
    return styles;
  };
}();
...
function parse_theme_xml(data, opts) {
  ...
  var t;
  var themes = {};
  if (!(t = str_match_xml(data, "a:themeElements")))
  ...
  if ((!opts || opts.cellText !== false) && p.t !== "z")
    try {
      ...
      if (p.t === "e")
        ...
        p.w = SSF_format(fmtid, datenum(p.v, !!date1904), _ssfopts);
      else
...
function parse_ws_xml(data, opts, idx, rels, wb, themes, styles) {
  ...
  if (mtch)
    parse_ws_xml_data(mtch[1], s, opts, refguess, themes, styles, wb);
  ...
  var m;
  if (m = data2.match(/legacyDrawing r:id="(.*?)"/))
    s["!legrel"] = m[1];
  if (opts && opts.nodim)
    ...
    switch (RT) {
      ...
      case 62:
        ...
        if (cf = styles.CellXf[val[0].iStyleRef])
          safe_format(p, cf.numFmtId, null, opts, themes, styles, date1904 > 0);
  ...
  switch (cell.t) {
    ...
    case "n":
      if (cell.v == (cell.v | 0) && cell.v > -1e3 && cell.v < 1e3) {
        ...
        if (last_seen)
          write_record(ba, 14, write_BrtShortError({ t: "e", v: 36 }, o));
        else
          write_record(ba, 3, write_BrtCellError({ t: "e", v: 36 }, o));
      } else if (!isFinite(cell.v)) {
        if (last_seen)
          write_record(ba, 14, write_BrtShortError({ t: "e", v: 7 }, o));
        else
          write_record(ba, 3, write_BrtCellError({ t: "e", v: 7 }, o));
      } else {
        if (last_seen)
    ...
    if (R && R.f) {
      ...
      switch (RecordType) {
        ...
        case 2:
          {
            if (out["!type"] == "chart") {
              if (options.dense ? (out["!data"][val.r] || [])[val.c] : out[encode_col(val.c) + encode_row(val.r)])
                ++val.c;
            }
        ...
        case 28:
          {
            ...
            if (opts.biff <= 5 && opts.biff >= 2)
              cmnt = { a: "SheetJ5", t: val[1] };
            else {
              var noteobj = objects[val[2]];
              cmnt = { a: val[1], t: noteobj.TxO.t };
              if (val[3] != null && !(val[3] & 2))
                cc.c.hidden = true;
            }
            cc.c.push(cmnt);
          }
...
function write_biff2_buf(wb, opts) {
  ...
  o._BIFF2FmtTable.forEach(function(f) {
    if (o.biff <= 3)
      write_biff_rec(ba, 30, write_BIFF2Format(f));
    else
      write_biff_rec(ba, 1054, write_BIFF4Format(f));
  });
...
function write_MsoDrawingGroup() {
  ...
  buf.write_shift(4, 74 + 8 * b8ocnts.length);
  {
    ...
    buf.write_shift(4, 16 + 8 * b8ocnts.length);
    {
      ...
      var acc = 0;
      for (var i = 0; i < b8ocnts.length; ++i)
        acc += b8ocnts[i] && b8ocnts[i][1] || 0;
  ...
  if (cell.bf)
    ...
    switch (cell.t) {
      ...
      case "n":
        var v = cell.t == "d" ? datenum(parseDate(cell.v, date1904), date1904) : cell.v;
        if (isNaN(v))
          write_biff_rec(ba, 517, write_BoolErr(R, C, 36, os, opts, "e"));
        else if (!isFinite(v))
          write_biff_rec(ba, 517, write_BoolErr(R, C, 7, os, opts, "e"));
        else
...
function write_biff8_global(wb, bufs, opts) {
  ...
  if (b8 && b8ocnts.length)
    write_biff_rec(C, 235, write_MsoDrawingGroup());
  if (b8 && opts.Strings)
...
var write_content_ods = /* @__PURE__ */ function() {
  ...
  return function wcx(wb, opts) {
    ...
    for (var i = 0; i != wb.SheetNames.length; ++i)
      o.push(write_ws(wb.Sheets[wb.SheetNames[i]], wb, i, opts, nfs, ((wb.Workbook || {}).WBProps || {}).date1904));
    if ((wb.Workbook || {}).Names)
...
function parse_sheet_legacy_drawing(sheet, type, zip, path, idx, opts, wb, comments) {
  ...
  if (draw)
    parse_vml(utf8read(draw), sheet, comments || []);
}
    ...
    for (i = 0; i != props.Worksheets; ++i) {
      ...
      if (wbrels && wbrels[i]) {
        ...
        if (!safegetzipfile(zip, path))
          path = wbrels[i][1];
        if (!safegetzipfile(zip, path))
          path = wbrelsfile.replace(/_rels\/[\S\s]*$/, "") + wbrels[i][1];
        stype = wbrels[i][2];
      } else {
